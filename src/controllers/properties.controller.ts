 ...
 * @param res Response
 */
export const storeProperty = async (req: Request, res: Response) => {
    return tryCatch(async () => {
        const validatedData = propertySchema.parse(req.body);
        const images = await uploadImages(validatedData.images as Express.Multer.File[]);
        const user = req.user;
        
        if (!user)
            return res.status(401).json({ success: false, message: 'Unauthorized' });

        const property = await createProperty(user.id, { ...validatedData, images });
...
export const updateProperty = async (req: Request, res: Response) => {
    return tryCatch(async () => {
        const { id } = req.params;
        const validatedData = propertySchema.parse(req.body);
        const images = await uploadImages(validatedData.images);
        const user = req.user;
...
export const deleteProperty = async (req: Request, res: Response) => {
    return tryCatch(async () => {
        ...
        const property = await deletePropertyModel(id);
